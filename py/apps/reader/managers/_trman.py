...
class AtlasTranslator(OfflineMachineTranslator):
  ...
  def translate(self, text, to='en', fr='ja', async=False, emit=False):
    ...
    if text:
      try:
        text = self._translate(emit, text, self.engine.translate, async=async)
        if text:
          # ATLAS always try to append period at the end
          text = wide2thin(text) #.replace(u". 。", ". ").replace(u"。", ". ").replace(u" 」", u"」").rstrip()
          text = self._unescapeTranslation(text, to=to, emit=emit)
          text = text.replace(u" 」", u"」") # remove the trailing space
...
class LecTranslator(OfflineMachineTranslator):
  ...
  def translate(self, text, to='en', fr='ja', async=False, emit=False):
    ...
    if text:
      try:
        #y = cachedYaku if cached else yaku
        #sub = y(text) if not async else skthreads.runsync(
        #    partial(y, text),
        #    parent=self.q)
        text = self._translate(emit, text, self.engine.translate, async=async)
        if text:
          text = wide2thin(text) #.replace(u"。", ". ").replace(u" 」", u"」").rstrip()
          text = self._unescapeTranslation(text, to=to, emit=emit)
          return text, to, self.key
      #except RuntimeError, e:
      except Exception, e:
...
class DreyeTranslator(OfflineMachineTranslator):
  ...
  def translate(self, text, to='zhs', fr='ja', async=False, emit=False):
    ...
    if text:
      try:
        text = self._translate(emit, text, engine.translate, async=async, to=to, fr=fr)
        if text:
          if to != 'zhs':
            text = zhs2zht(text)
...
class InfoseekTranslator(OnlineMachineTranslator):
  ...
  def translate(self, text, to='en', fr='ja', async=False, emit=False):
    ...
    if fr != 'ja':
      return None, None, None
    to = 'en' if to in ('ru', 'nl', 'pl') else to
    if emit:
      self.emitLanguages(fr=fr, to=to)
    text = self._escapeText(text, to=to, emit=emit)
    if text:
...
class ExciteTranslator(OnlineMachineTranslator):
  ...
  def translate(self, text, to='en', fr='ja', async=False, emit=False):
    ...
    if fr != 'ja':
      return None, None, None
    to = 'en' if to in ('id', 'th', 'vi', 'nl', 'pl') else to
    if emit:
      self.emitLanguages(fr=fr, to=to)
    text = self._escapeText(text, to=to, emit=emit)
    if text:
  ...
  def translate(self, text, to='zhs', fr='ja', async=False, emit=False):
    ...
    if text:
      ...
      if text:
        if to == 'zht':
          text = zhs2zht(text)
        text = self.__baidu_repl_after(text)
        text = self._unescapeTranslation(text, to=to, emit=emit)
    return text, to, self.key
