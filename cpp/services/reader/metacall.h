...
signals:
  ...
  void serverMessageRequested(const QString &type, const QString &data);
  void serverMessageReceived(const QString &type, const QString &data);

  // Message sent from server to vnragent
  void agentMessageRequested(const QString &type, const QString &data);
  void agentMessageReceived(const QString &type, const QString &data);

public:
  explicit ReaderMetaCallPropagator(QObject *parent = nullptr)
    : Base(parent)
  {
    connect(this, SIGNAL(serverMessageRequested(QString,QString)), SIGNAL(serverMessageReceived(QString,QString)), Qt::QueuedConnection);
    connect(this, SIGNAL(agentMessageRequested(QString,QString)), SIGNAL(agentMessageReceived(QString,QString)), Qt::QueuedConnection);
  }
};
