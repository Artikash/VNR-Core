...
#define WINIME_CLASS_JA L"MSIME.Japan"
#define WINIME_CLASS_CN L"MSIME.China"
#define WINIME_CLASS_TW L"MSIME.Taiwan"
...
typedef boost::function<void (const wchar_t *, size_t)> winime_apply_func_t;
bool winime_apply(winime_t *ife, unsigned long req, unsigned long mode,
                  const winime_apply_func_t &func, const wchar_t *src, size_t len = 0);

// Get each result, similar to list.collect in groovy. Return if succeed.
typedef boost::function<void (const wchar_t *, size_t, const wchar_t *, size_t)> winime_collect_func_t;
bool winime_collect(winime_t *ife, unsigned long req, unsigned long mode,
                    const winime_collect_func_t &func, const wchar_t *src, size_t len = 0);
