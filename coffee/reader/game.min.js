(function(){var n,k,p,q,d,g,r,s,t,u,v,w,x,y,e=function(a,b){return function(){return a.apply(b,arguments)}};d=function(){};n=Haml('%a(href="#{url}" title="#{url}")\n  %img.img-rounded.zoom.zoom-cg(src="#{url}")');y=function(a){return n({url:a})};x=function(){return gameBean.getSampleImages().split(",").map(y).join("")};q=function(a){return twttr.widgets.createTimeline(a.id,a.el,a.callback,a.options)};this.initAmazonReview=function(){var a;d("initAmazonReview: enter");a=$("iframe.amazon");a.load(function(){var b,
...
function(a){return $(".youtube:not(.iframe)").width(this.youtubeWidth*a)};a.prototype.youtubeFrameWidth=function(){return Math.min(480*this.ratio,Math.max(480,this.maxWidth()))};a.prototype.youtubeFrameHeight=function(){return 9*this.youtubeFrameWidth()/16};return a}();u=function(){var a;d("bindToolbar: enter");this.zoomer=new p(this.viewBean);a=new ZoomSlider(function(a){return function(d){return a.zoomer.zoom(1+2*d)}}(this));new Toolbar(this.viewBean,{width:60,height:244,move:a.reloadOffset});return d("bindToolbar: leave")};
t=function(){d("bindRuby: enter");$.fn.inject=function(){return this.each(function(){return window.injectruby(this)})};$(".ruby").inject();return d("bindRuby: leave")};s=function(){var a;a=$("section.cg");return a.find("input.switch").bootstrapSwitch().on("switchChange.bootstrapSwitch",function(b,d){var c;c=a.find(".images");return d?c.hasClass("rendered")?c.fadeIn():c.hide().html(x()).addClass("rendered").fadeIn().masonry({itemSelector:"img",isFitWidth:!0}).imagesLoaded(function(){c.find("img").width(220*
k);return c.masonry()}):c.fadeOut()})};w=function(){var a;a=$("section.twitter");return a.find("input.switch").bootstrapSwitch().on("switchChange.bootstrapSwitch",function(b,d){var c,e,f,l,k,g,m;if(window.twttr)if(c=a.find(".widgets"),d){if(c.hasClass("rendered"))return c.fadeIn();c.show().addClass("rendered");if(e=gameBean.getTwitterWidgets()){g=e.split(",");m=[];l=0;for(k=g.length;l<k;l++)f=g[l],e=document.createElement("div"),e.className="timeline",c.append(e),m.push(q({id:f,el:e,options:{lang:"ja",
width:300,height:500,chrome:"transparent noborders noheader",showReplies:!0}}));return m}}else return c.fadeOut()})};r=function(){s();return w()};v=function(){return $(".tts").click(function(){return ttsBean.speak(this.dataset.text)})};g=function(){if(null==this.gameBean)return d("init: wait"),setTimeout(g,100);d("init: enter");u();r();v();t();return d("init: leave")};$(function(){return g()})}).call(this);
