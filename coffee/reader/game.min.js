...
return k("bindToolbar: leave")};T=function(){return $.fn.annotate=function(){this.each(function(){return window.annotate(this)});return this}};n=function(){if($(document.body).is("[class |= annot-opt]"))return $(".annot:not(.annot-root)").annotate()};P=function(a){return twttr.widgets.createTimeline(a.id,a.el,a.callback,a.options)};ta=function(a){return HAML_SAMPLE_IMAGE({url:a})};ka=function(a){a=gameBean.getSampleImages(a).split(",");return[a.length,a.map(ta).join("")]};va=function(a){null==a.date&&
(a.date="");a.img=gameBean.getYouTubeImageUrl(a.vid,!0);return HAML_VIDEO(a)};pa=function(){return JSON.parse(gameBean.getVideos()).map(va).join("")};oa=function(a){return'<iframe width="480" height="360" src="http://youtube.com/embed/'+a+'" frameborder="0" allowfullscreen />'};ma=function(a){return HAML_SETTINGS({file:a})};na=function(a){var b,c,d;c=0;for(d=a.length;c<d;c++)b=a[c],b.langName=util.getLangName(b.lang)||"*",b.userAvatar&&(b.userAvatarUrl=util.getAvatarUrl(b.userAvatar));return HAML_USERS({users:a})};
ja=function(a){return"scape"===a?la():gameBean.getReview(a)};s=function(a){return null!=a?a.replace(/\n/g,"<br/>"):void 0};p=0;ua=function(a){var b,c,d,f;++p;try{return c=0,a.okazu_tokuten&&0>a.okazu_tokuten&&-10<a.okazu_tokuten&&(c=-a.okazu_tokuten),b=d="",a.memo&&(b=s(a.memo),200<a.memo.length&&(d=s(a.memo.slice(0,201)+" \u2026\u2026"))),HAML_SCAPE_REVIEW({count:p,user:a.uid,userUrl:"http://erogamescape.dyndns.org/~ap2/ero/toukei_kaiseki/user_infomation.php?user="+a.uid,date:a.timestamp,netabare:a.netabare,
score:a.tokuten||0,ecchiScore:c,title:s(a.hitokoto),content:b,contentLength:null!=(f=a.memo)?f.length:void 0,lessContent:d})}catch(e){return k(e),'<div class="entry entry-empty"/>'}};z=function(a,b){var c,d;return(c=gameBean.getScapeReviews(a,b))?(c=JSON.parse(c),d=c.map(ua).join(""),[d,c.length]):["",0]};la=function(){var a,b;a=z(0,10);b=a[0];a=a[1];if(b)return HAML_SCAPE_REVIEWLIST({entries:b,empty:10>a})};ca=function(){return $.fn.spin.presets.section={left:"-8px",length:4,radius:4,lines:12,width:1}};
da=function(){aa();ba();ga();fa();return ha()};ba=function(){var a,b,c,d;c=$("section.settings");a=c.find(".contents");d=c.find(".spin");b=c.find(".msg");return c.find("input.switch").bootstrapSwitch().on("switchChange.bootstrapSwitch",function(c,e){if(e){if(a.hasClass("rendered"))return a.fadeIn();a.show().addClass("rendered");d.spin("section");return rest.forum.query("game",{data:{id:GAME_ID,select:"file"},error:function(){d.spin(!1);a.removeClass("rendered");return b.addClass("text-danger").text("("+
tr("Internet error")+")")},success:function(c){d.spin(!1);return c.file?(b.empty(),c=ma(c.file),a.hide().html(c).fadeIn()):b.removeClass("text-danger").text(" ("+tr("Empty")+")")}})}return a.fadeOut()})};ga=function(){var a,b,c,d;c=$("section.users");a=c.find(".contents");d=c.find(".spin");b=c.find(".msg");return c.find("input.switch").bootstrapSwitch().on("switchChange.bootstrapSwitch",function(c,e){if(e){if(a.hasClass("rendered"))return a.fadeIn();a.show().addClass("rendered");d.spin("section");
return rest.forum.query("game",{data:{id:GAME_ID,select:"users"},error:function(){d.spin(!1);a.removeClass("rendered");return b.addClass("text-danger").text("("+tr("Internet error")+")")},success:function(c){d.spin(!1);return(c=c.users||c.subs)?(b.empty(),c=na(c),a.hide().html(c).fadeIn().find("a.user-link").click(function(){var a;a=$(this).closest(".user").data("name");return mainBean.showUser(a)})):b.removeClass("text-danger").text(" ("+tr("Empty")+")")}})}return a.fadeOut()})};m=[];S=function(a){return _.findWhere(m,
{id:a})};A=function(){return(m.length-0)%20};aa=function(){var a,b,c,d,f,e,g;f=$("section.stats");b=f.find(".contents");c=b.find(".footer");a=b.find(".topics");e=f.find(".spin");d=f.find(".msg");f.find("input.switch").bootstrapSwitch().on("switchChange.bootstrapSwitch",function(f,wa){if(wa){if(b.hasClass("rendered"))return b.fadeIn();b.show().addClass("rendered");e.spin("section");return rest.forum.list("topic",{data:{type:"review",subjectId:GAME_ID,subjectType:"game",sort:"updateTime",asc:!1,limit:20,
complete:!0},error:function(){e.spin(!1);b.removeClass("rendered");return d.addClass("text-danger").text("("+tr("Internet error")+")")},success:function(b){e.spin(!1);return b.length?(d.empty(),b=y(b),a.hide().html(b).fadeIn(),A()?c.html(HTML_NOMORE):(c.html(HTML_MORE),c.find(".btn-more").click(g)),u()):d.removeClass("text-danger").text(" ("+tr("Empty")+")")}})}return b.fadeOut()});return g=function(){return rest.forum.list("topic",{data:{type:"review",subjectId:GAME_ID,subjectType:"game",sort:"updateTime",
asc:!1,first:m.length,limit:20,complete:!0},error:function(){e.spin(!1);return growl.warn(tr("Internet error"))},success:function(b){e.spin(!1);b.length&&(b=y(b),a.append(b));A()&&c.html(HTML_NOMORE);return u()}})}};y=function(a){m.push.apply(m,a);try{return a.map(sa).join("")}catch(b){return k(b),'<div class="entry entry-empty"/>'}};sa=function(a){return HAML_REVIEW_TOPIC({id:a.id,userName:a.userName,userStyle:a.userColor?"color:"+a.userColor:"",lang:util.getLangName(a.lang),userAvatarUrl:util.getAvatarUrl(a.userAvatar),
content:util.renderContent(a.content),createTime:util.formatDate(a.createTime),updateTime:a.updateTime>a.createTime?util.formatDate(a.updateTime):"",image:a.image?{title:a.image.title,url:util.getImageUrl(a.image)}:null,scores:a.scores,likeCount:a.likeCount||0,dislikeCount:a.dislikeCount||0,postCount:a.postCount})};u=function(){return $(".topic.topic-new").each(function(){var a,b,c,d;b=$(this).removeClass("topic-new");d=b.data("id");c=S(d);a=b.find("> .right > .footer");a.find(".btn-edit").click(function(){c&&
editTopic(c);return!1});a.find(".btn-reply").click(function(){replyTopic(d);return!1});a.find(".btn-posts").click(function(){$(this).remove();ia(d);return!1});(null!=c&&c.likeCount||null!=c&&c.dislikeCount)&&a.find(".like-group").removeClass("fade-in");a.find(".btn.like").click(function(){var b,e,g,l;c&&c.userName!==USER_NAME&&(b=a.find(".btn.dislike.selected"),b.length&&(b.removeClass("selected"),g=b.find(".value"),g.text(-1+Number(g.text()))),e=$(this),e.parent(".like-group").removeClass("fade-in"),
